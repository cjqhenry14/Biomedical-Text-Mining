...
def readDataTest(year_name, xml_doc):
    yearTopicWordsFile = open("art_topic/" + year_name + '.txt', 'a+')

    contentFileName = "art_content/" + year_name + "/" + xml_doc
...
'''def linetest():
    yearTopicWordsFile = open("art_topic/t.txt", 'a+')

    contentFileName = "1468-6708-3-10.xml"
    file = open(contentFileName)
    ...
    while 1:
        ...
        try:
            np_extractor = NPExtractor(line)
            np_extractor.extract("2001-2002", contentFileName, yearTopicWordsFile)
        except:
...
def scanDir(year_name):
    yearTopicWordsFile2 = open("art_topic/" + year_name + '.txt', 'w+')#clean the file
    file_num = 1000
    yearTopicWordsFile2.write(str(file_num) + '\n')
    yearTopicWordsFile2.close()

    file_list = os.listdir('art_content/' + year_name)
